...
const socialLinks = [
  ...
  export const Contact = () => {
    ...
    const onSubmit = async (event) => {
      ...
      formData.append("access_key", "770236aa-68de-4850-8b2c-3181e2a54669"); // known working key
      ...
      try {
        ...
        if (data.success) {
          setResult(t("contact.success") || "Email sent successfully.");
          event.target.reset();
        } else {
          setResult(t("contact.error") || data.message || "Something went wrong.");
        }
      } catch (error) {
        console.error("Fetch error:", error);
        setResult(t("contact.error") || "Submission failed.");
      }
    };
  
    return (
      <section id="contact" className="section">
        <div className="container lg:grid lg:grid-cols-2 lg:items-stretch mb-8">
          <div className="mb-12 lg:mb-0 lg:flex lg:flex-col">
            ...
            <p className="text-zinc-400 mt-3 mb-8 max-w-[50ch] lg:max-w-[30ch] reveal-up">
              {t("contact.subtitle") || "Let us know how we can help."}
            </p>
            ...
            <div className="flex items-center gap-2 mt-auto">
              {socialLinks.map(({ href, icon }, key) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="w-12 h-12 grid place-items-center ring-inset ring-2 ring-zinc-50/5 rounded-lg transition hover:bg-zinc-50 hover:text-zinc-950"
                >
                  {icon}
                </a>
              ))}
          ...
          <form onSubmit={onSubmit} className="xl:pl-10 2xl:pl-20">
            ...
            >
              {result || t("contact.submit") || "Send Message"}
            </button>
          </form>
        </div>
      </section>
